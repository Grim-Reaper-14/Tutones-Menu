...
namespace Tutones::Game::PersonalVehicles
{
    ...
    // GiveVehicleReward must be driven once per GTA script/scheduler generation for
    // the entire lifetime of the garage frontend. ControlStatus == 3 means the GTA
    // selector is active; it is NOT a signal to stop calling the function.
    //
    // The flow is complete only when GiveVehicleReward itself returns true while
    // ControlStatus is no longer 3. This mirrors the lifecycle expected by
    // AM_MP_VEHICLE_REWARD and prevents the selector from being repeatedly created
    // and destroyed while the player is trying to choose a garage or slot.
    ...
    private:
        ...
        static constexpr auto FlowTimeout = std::chrono::seconds(90);
        ...
        void BeginOnGameThread()
        {
            ...
            // Clear stale data once before starting. Never clear these values again
            // while the reward script owns the garage selector.
            ...
            TUTONES_LOG_INFO(
                "vehicle.savepv",
                std::string("Starting stable AM_MP_VEHICLE_REWARD flow; threadId=")
        ...
        void RunRewardTick()
        {
            ...
            // nullopt means the VM invocation itself failed. A contained false value
            // is a normal in-progress result and must be driven again next script tick.
            if (!callResult)
                return Finish(false, "GiveVehicleReward script function could not be invoked");

            LogState("tick", *callResult, rewardData);
            ...
            if (rewardData->controlStatus == 3)
            {
                if (!m_SelectorObserved)
                {
                    ...
                    TUTONES_LOG_INFO(
                        "vehicle.savepv",
                        "GTA personal-garage selector is stable in ControlStatus 3");
                    SetPending("GTA garage selector active - choose a garage and slot");
                }

                // IMPORTANT: keep calling GiveVehicleReward while status == 3.
                // The script function owns the frontend lifecycle and needs another
                // invocation on the next scheduler generation to keep progressing.
                return QueueRewardTick();
            }

            if (*callResult)
            {
                // This is the real completion contract: GiveVehicleReward reports
                // completion AND GTA is no longer inside the selector state.
                const bool selectorWasShown = m_SelectorObserved;
                ...
                return Finish(
                    ...
                    selectorWasShown
                        ...
                        : "Vehicle-reward flow completed without opening a selector");
            }

            if (!m_SelectorObserved)
            {
                if (RewardPreparationStatePresent(*rewardData))
                    SetPending("GTA is preparing the personal-garage selector...");
                else
                    SetPending("Opening GTA personal-garage selector...");
            }
        ...
        [[nodiscard]] bool ValidateLiveFlow()
        {
            ...
            if (liveReward != m_Thread)
            {
                TUTONES_LOG_WARN(
                    "vehicle.savepv",
                    "AM_MP_VEHICLE_REWARD thread identity changed during garage save");
        ...
        void QueueRewardTick()
        {
            if (!m_Pending.load(std::memory_order_acquire))
                return;

            // GameRuntime snapshots its queue at the beginning of each scheduler
            // generation, so this self-requeue executes on a later GTA script tick and
            // cannot recursively spin inside the same callback.
            if (!Runtime::GameRuntime::Get().Enqueue([this] { RunRewardTick(); }))
        ...
        void LogState(const char* phase, int callResult, const Types::VehicleRewardData* data)
        {
            ...
            TUTONES_LOG_INFO(
                ...
                std::string(phase)
                    ...
                    + " calls=" + std::to_string(m_CallCount));
        }
