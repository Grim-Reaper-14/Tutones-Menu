...
namespace Tutones::UI
{
    namespace
    {
        ...
        bool OriginalTabButton(const char* id, const char* icon, bool selected, const ImVec2& localPos) noexcept
        {
            ...
            draw->AddRectFilled(ImVec2(max.x + 4.0f, min.y + 6.0f), ImVec2(max.x + 8.0f, max.y - 6.0f), ImGui::GetColorU32(ImVec4(Accent.x, Accent.y, Accent.z, anim.rectOpacity)), 4.0f);
            return pressed;
        }
        ...
        bool OriginalSubtabButton(const char* id, const char* label, bool selected, const ImVec2& localPos) noexcept
        {
            ...
            draw->AddRectFilled(ImVec2(max.x + 5.0f, min.y + 9.0f), ImVec2(max.x + 8.0f, max.y - 9.0f), ImGui::GetColorU32(ImVec4(Accent.x, Accent.y, Accent.z, anim.rectOpacity)), 4.0f);
            return pressed;
        }
        ...
        void DrawPanel(ImDrawList* draw, const ImVec2& windowPos, const ImVec2& localPos, const ImVec2& size, const char* title, std::initializer_list<const char*> lines) noexcept
        {
            ...
            for (const char* line : lines)
            {
                ...
                draw->AddText(FontOrDefault(g_Medium), 14.0f, ImVec2(min.x + 16.0f, y), Color(188, 188, 193), line);
                y += 22.0f;
                if (y > max.y - 18.0f)
                    break;
            }
    ...
    void TutonesMenu::RenderCategoryRail() noexcept
    {
        ...
        auto* draw = ImGui::GetWindowDrawList();
        draw->AddText(FontOrDefault(g_SubtabTitle), 15.0f, Offset(windowPos, 72.0f, 60.0f), Color(255, 255, 255, 102), "MAIN");
        ...
        draw->AddText(FontOrDefault(g_Medium), 12.0f, Offset(windowPos, 64.0f, 397.0f), Color(110, 110, 116), "ARROWS  MOVE");
        draw->AddText(FontOrDefault(g_Medium), 12.0f, Offset(windowPos, 64.0f, 416.0f), Color(110, 110, 116), "ENTER  SELECT");
        draw->AddText(FontOrDefault(g_Medium), 12.0f, Offset(windowPos, 64.0f, 435.0f), Color(110, 110, 116), "ESC  BACK");
    }
    ...
    void TutonesMenu::RenderContent() noexcept
    {
        ...
        {
        case 0:
            DrawPanel(draw, windowPos, ImVec2(226.0f, 16.0f), ImVec2(240.0f, 300.0f), "Tutones Menu", {"Player options", "Vehicle customization", "World controls", "Menu settings", "Mouse + keyboard control"});
            ...
            DrawPanel(draw, windowPos, ImVec2(476.0f, 272.0f), ImVec2(240.0f, 174.0f), "Favorites", {"Quick access slots", "Pinned options", "Recent selections", "Custom shortcuts"});
            break;

        case 1:
            DrawPanel(draw, windowPos, ImVec2(226.0f, 16.0f), ImVec2(240.0f, 300.0f), "Player", {"Health", "Armor", "Wanted level", "Invincibility", "Visibility", "Player model"});
            DrawPanel(draw, windowPos, ImVec2(476.0f, 16.0f), ImVec2(240.0f, 240.0f), "Movement", {"Run speed", "Swim speed", "Super jump", "No ragdoll", "Movement options"});
            DrawPanel(draw, windowPos, ImVec2(226.0f, 332.0f), ImVec2(240.0f, 114.0f), "Selected", {selected});
            DrawPanel(draw, windowPos, ImVec2(476.0f, 272.0f), ImVec2(240.0f, 174.0f), "Appearance", {"Visibility", "Model", "Outfit", "Appearance options"});
            break;
        ...
        default:
            ...
            DrawPanel(draw, windowPos, ImVec2(476.0f, 16.0f), ImVec2(240.0f, 240.0f), "Theme", {"Charcoal layout", "Green accent", "Original icon font", "Original 730 x 460 design"});
            DrawPanel(draw, windowPos, ImVec2(226.0f, 332.0f), ImVec2(240.0f, 114.0f), "Selected", {selected});
