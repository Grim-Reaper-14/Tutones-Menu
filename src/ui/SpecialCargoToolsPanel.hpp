...
namespace Tutones::UI
{
    inline void RenderSpecialCargoToolsControl() noexcept
    {
        ...
        // Recovery -> Businesses is rendered in its own ImGui child window. A launcher
        // submitted to the parent draw list can be covered by that child, so render the
        // launcher as a tiny top-level overlay above the lower-right corner of the
        // Businesses panel. This keeps the control visible without changing the existing
        // Recovery layout or its scrolling behavior.
        const ImVec2 hostPos = ImGui::GetWindowPos();
        ImGui::SetNextWindowPos(ImVec2(hostPos.x + 560.0f, hostPos.y + 365.0f), ImGuiCond_Always);
        ...
        if (ImGui::Begin("##special_cargo_tools_launcher", nullptr, launcherFlags))
        {
            ImGui::BeginDisabled(state.pending);
            if (ImGui::Button("Special Cargo Tools...", ImVec2(-1.0f, 0.0f)))
